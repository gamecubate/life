...
ig.baked=true;ig.module('game.rule-set').requires('impact.game').defines(function(){RuleSet=ig.Class.extend({map:null,init:function(aMap)
{this.map=aMap;},step:function()
{},zombifyAll:function(aMap)
{for(var row=0;row<aMap.height;row++)
for(var col=0;col<aMap.width;col++)
aMap.data[row][col]=ig.Entity.DEAD;},dezombify:function(n)
{this.zombifyAll(this.map);var count=0;do
{var col=Math.floor(Math.random()*this.map.width);var row=Math.floor(Math.random()*this.map.height);if(this.map.data[row][col]&ig.Entity.DEAD)
{this.map.data[row][col]=ig.Entity.AWAKE;count+=1;}}while(count<n);},stateAt:function(col,row,aMap)
{if(aMap.data[row])
return aMap.data[row][col];},stateAtXY:function(x,y,aMap)
{return aMap.getTile(x,y);},neighborsOf:function(col,row,aMap)
{var states=[];var state;state=this.stateAt(col-1,row-1,aMap);if(state!=undefined)states[states.length]=state;state=this.stateAt(col,row-1,aMap);if(state!=undefined)states[states.length]=state;state=this.stateAt(col+1,row-1,aMap);if(state!=undefined)states[states.length]=state;state=this.stateAt(col-1,row,aMap);if(state!=undefined)states[states.length]=state;state=this.stateAt(col+1,row,aMap);if(state!=undefined)states[states.length]=state;state=this.stateAt(col-1,row+1,aMap);if(state!=undefined)states[states.length]=state;state=this.stateAt(col,row+1,aMap);if(state!=undefined)states[states.length]=state;state=this.stateAt(col+1,row+1,aMap);if(state!=undefined)states[states.length]=state;return states;},foundWithState:function(state,states)
{var found=0;for(var i=0;i<states.length;i++)
{var theState=states[i];if(theState&state)
{found+=1;}}
return found;}});RuleSetConway1=RuleSet.extend({step:function()
{var destinies=ig.copy(this.map.data);for(var row=0;row<this.map.height;row++)
{for(var col=0;col<this.map.width;col++)
{var state=this.stateAt(col,row,this.map);var neighbors=this.neighborsOf(col,row,this.map);var liveNeighbors=this.foundWithState(ig.Entity.AWAKE,neighbors);if(state&ig.Entity.AWAKE&&liveNeighbors<2)
...
{destinies[row][col]=ig.Entity.AWAKE;}}}
this.map.data=destinies;this.parent();}});});
...
{if(!this.loaded)
{return;}
ig.system.context.drawImage(this.data,0,0);}});GameOfLife=ig.Game.extend({clearColor:null,state:null,COLS:40,ROWS:60,ruleset:null,simulationTimer:null,stepDuration:0.05,init:function()
{new ig.Symbols("PLAYING SLEEPING AWAKE DEAD");var bgData=this.makeData(this.COLS,this.ROWS,5);var bgMap=new ig.BackgroundMap(8,bgData,new ig.Image('./media/states.png'));bgMap.preRender=true;this.backgroundMaps.push(bgMap);var cellStates=this.makeData(this.COLS,this.ROWS);var cellsMap=new ig.BackgroundMap(8,cellStates,new ig.Image('./media/states.png'));this.backgroundMaps.push(cellsMap);this.ruleset=new RuleSetConway1(cellsMap);this.ruleset.dezombify(Math.round(this.COLS*this.ROWS/6));this.simulationTimer=new ig.Timer();ig.input.bind(ig.KEY._1,'1');ig.input.bind(ig.KEY._2,'2');ig.input.bind(ig.KEY._3,'3');ig.input.bind(ig.KEY._4,'4');ig.input.bind(ig.KEY._5,'5');ig.input.bind(ig.KEY.MOUSE1,'mouse1');this.state=ig.Entity.PLAYING;},makeData:function(cols,rows,tileIndex)
{tileIndex=(tileIndex||0);var aMap=[];for(var i=0;i<rows;i++)
{aMap[i]=[];for(var j=0;j<cols;j++)
aMap[i][j]=tileIndex;}
return aMap;},update:function()
...
{this.ruleset.step();this.simulationTimer.set(this.stepDuration);}}},handleKeys:function()
...
this.ruleset.dezombify(this.COLS*this.ROWS/6);},draw:function()
