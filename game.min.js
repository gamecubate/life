...
{var state=this.stateAt(col,row,data);var neighbors=this.neighborsOf(col,row,data);var liveNeighbors=this.foundWithState(ig.Entity.ALIVE,neighbors);if(state&ig.Entity.ALIVE&&liveNeighbors<2)
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.background-map','game.rule-set','plugins.symbols.symbols','plugins.impact-splash-loader').defines(function(){GameOfLife=ig.Game.extend({clearColor:null,COLS:40,ROWS:60,cellsMap:null,ruleset:null,simulationTimer:null,stepDuration:0.1,init:function()
{new ig.Symbols("ALIVE DEAD");var bgData=this.createMapData(this.COLS,this.ROWS,5);var bgMap=new ig.BackgroundMap(8,bgData,new ig.Image('media/states.png'));bgMap.preRender=true;this.backgroundMaps.push(bgMap);var cellStates=this.createPopulation(this.COLS,this.ROWS,0.15);this.cellsMap=new ig.BackgroundMap(8,cellStates,new ig.Image('media/states.png'));this.backgroundMaps.push(this.cellsMap);this.ruleset=new RuleSetConway1();this.simulationTimer=new ig.Timer();ig.input.bind(ig.KEY._1,'1');ig.input.bind(ig.KEY._2,'2');ig.input.bind(ig.KEY._3,'3');ig.input.bind(ig.KEY._4,'4');ig.input.bind(ig.KEY._5,'5');ig.input.bind(ig.KEY.MOUSE1,'mouse1');},createMapData:function(cols,rows,tileIndex)
{var index=(tileIndex|0);var data=[];for(var row=0;row<rows;row++)
{data[row]=[];for(var col=0;col<cols;col++)
data[row][col]=index;}
return data;},createPopulation:function(cols,rows,aliveRatio)
{var pop=[];for(var row=0;row<rows;row++)
{pop[row]=[];for(var col=0;col<cols;col++)
{if(Math.random()<aliveRatio)
pop[row][col]=ig.Entity.ALIVE;else
...
this.cellsMap.data=this.createPopulation(this.COLS,this.ROWS,0.15);},draw:function()
