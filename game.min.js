...
// lib/game/rule-set.js
ig.baked=true;ig.module('game.rule-set').requires('impact.game').defines(function(){RuleSet=ig.Class.extend({step:function(data,cols,rows)
{},stateAt:function(col,row,data)
{if(data[row])
return data[row][col];},neighborsOf:function(col,row,data)
{var neighbors=[];var state;state=this.stateAt(col-1,row-1,data);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col,row-1,data);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col+1,row-1,data);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col-1,row,data);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col+1,row,data);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col-1,row+1,data);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col,row+1,data);if(state!=undefined)neighbors[neighbors.length]=state;state=this.stateAt(col+1,row+1,data);if(state!=undefined)neighbors[neighbors.length]=state;return neighbors;},foundWithState:function(state,states)
{var found=0;for(var i=0;i<states.length;i++)
{var theState=states[i];if(theState&state)
{found+=1;}}
return found;}});RuleSetConway1=RuleSet.extend({step:function(data,cols,rows)
{var nextStates=ig.copy(data);for(var row=0;row<rows;row++)
{for(var col=0;col<cols;col++)
{var state=this.stateAt(col,row,data);var neighbors=this.neighborsOf(col,row,data);var liveNeighbors=this.foundWithState(ig.Entity.AWAKE,neighbors);if(state&ig.Entity.AWAKE&&liveNeighbors<2)
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.background-map','game.rule-set','plugins.symbols.symbols','plugins.impact-splash-loader').defines(function(){FullsizeBackdrop=ig.Image.extend({resize:function(){},draw:function()
{if(!this.loaded)
{return;}
ig.system.context.drawImage(this.data,0,0);}});GameOfLife=ig.Game.extend({clearColor:null,state:null,COLS:40,ROWS:60,cellsMap:null,ruleset:null,simulationTimer:null,stepDuration:0.1,init:function()
{new ig.Symbols("PLAYING SLEEPING AWAKE DEAD");var bgData=this.createMapData(this.COLS,this.ROWS,5);var bgMap=new ig.BackgroundMap(8,bgData,new ig.Image('media/states.png'));bgMap.preRender=true;this.backgroundMaps.push(bgMap);var cellStates=this.createPopulation(this.COLS,this.ROWS,0.13);this.cellsMap=new ig.BackgroundMap(8,cellStates,new ig.Image('media/states.png'));this.backgroundMaps.push(this.cellsMap);this.ruleset=new RuleSetConway1();this.simulationTimer=new ig.Timer();ig.input.bind(ig.KEY._1,'1');ig.input.bind(ig.KEY._2,'2');ig.input.bind(ig.KEY._3,'3');ig.input.bind(ig.KEY._4,'4');ig.input.bind(ig.KEY._5,'5');ig.input.bind(ig.KEY.MOUSE1,'mouse1');this.state=ig.Entity.PLAYING;},createMapData:function(cols,rows,tileIndex)
{var index=(tileIndex|0);var data=[];for(var row=0;row<rows;row++)
{data[row]=[];for(var col=0;col<cols;col++)
data[row][col]=index;}
return data;},createPopulation:function(cols,rows,liveRatio)
{var pop=[];for(var row=0;row<rows;row++)
{pop[row]=[];for(var col=0;col<cols;col++)
{if(Math.random()<liveRatio)
pop[row][col]=ig.Entity.AWAKE;else
pop[row][col]=ig.Entity.DEAD;}}
return pop;},update:function()
{this.updateSimulation();this.handleKeys();this.handleMouse();this.parent();},updateSimulation:function()
{if(this.state&ig.Entity.PLAYING)
{if(this.simulationTimer.delta()>0&&this.stepDuration>0)
{this.cellsMap.data=this.ruleset.step(this.cellsMap.data,this.cellsMap.width,this.cellsMap.height);this.simulationTimer.set(this.stepDuration);}}},handleKeys:function()
{if(ig.input.pressed('1'))
this.stepDuration=1;else if(ig.input.pressed('2'))
this.stepDuration=0.5;else if(ig.input.pressed('3'))
this.stepDuration=0.25;else if(ig.input.pressed('4'))
this.stepDuration=0.1;else if(ig.input.pressed('5'))
this.stepDuration=0.05;},handleMouse:function()
{if(ig.input.released("mouse1"))
this.cellsMap.data=this.createPopulation(this.COLS,this.ROWS,0.13);},draw:function()
{ig.system.context.clearRect(0,0,ig.system.realWidth,ig.system.realHeight);this.parent();},});ig.main('#canvas',GameOfLife,30,320,480,1,ig.ImpactSplashLoader);});